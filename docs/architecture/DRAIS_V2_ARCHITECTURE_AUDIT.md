...
**Remaining for Phase 7, not started:** `enrollments`, `subjects`/`terms`/`academic_years`/`departments`/`subject_groups`/`class_subjects` (the rest of the academic-structure join `report_snapshots` generation needs), `report_snapshots` + DRCE's render-path reads, fee tables (last, per the stated priority order), the salary/bank-account fields deliberately deferred from `staff` above (revisit once repo-sqlite has SQLCipher-backed at-rest encryption), and the `db-mode.ts` three-way-mode wiring itself.
