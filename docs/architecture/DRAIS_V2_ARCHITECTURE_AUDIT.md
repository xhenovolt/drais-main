...
**🟢 Sub-effort 11 of several — the first complete offline module: students — done (2026-08-22).** The first user-facing feature (not just login) to actually work offline, end to end, through real routes and a real page. Follows directly from a course correction of its own: the user's stated goal was "at least one module working... not everything at once," and this session's first instinct — matching the real `/students/list` page — was wrong before any code was written, caught by an investigation, not assumption.

**What the investigation found, and why it changed the plan:** "students management" is not simple CRUD. It's **~55 API routes**. The real list page (3,160 lines) doesn't query `students` directly at all — it's driven by `/api/students/enrolled`, a join across `enrollments`/`classes`/`streams`/`academic_years`/`terms`/`programs` plus live fee balances from `student_ledger`. The detail page spans 10+ tables in one endpoint. Student deletion has **at least four inconsistent code paths** — only one goes through the real Trash system properly (audit trail + biometric-device suspension); the others bypass it with hand-rolled SQL. There's a genuine live inconsistency in the app itself (some code treats a student's class as living on `students.class_id`, other code insists it only lives in `enrollments` — flagged, not resolved by guessing). None of that is a reasonable first offline target — replicating it would have meant building `EnrollmentRepo` (already deferred once, sub-effort 4, as "a materially bigger sub-effort on its own") plus fees plus a from-scratch decision about which of four delete implementations is "correct," before anything shipped.

**Confirmed scope instead (2026-08-22, user, AskUserQuestion): a genuinely smaller, new screen** — view/add/edit a student's core identity only (name, DOB, gender, contact, admission number, status, notes). Not a port of the real list page; a deliberately different, smaller thing.

**Architecture decision, consistent with §25a but taken one step further:** rather than grafting even an additive mode-branch into the existing 3,160-line `list/page.tsx`, this is entirely new, separate routes and a separate page (`src/app/api/students/offline/*`, `src/app/students/offline/page.tsx`) — zero lines of the real student routes touched. Given how large and tightly-coupled that page is, branching inside it — even additively — was judged riskier than just not touching it at all.

**Real gaps closed in `StudentRepo` before building on it:** the interface had no `restore()` method at all, and `StudentRecord` was missing the `deleted_by`/`delete_reason`/`restored_at`/`restored_by` audit trail — confirmed as real columns via the investigation's own reading of the trash registry's `displaySelect` SQL (`e.deleted_by, e.delete_reason, e.restored_at, e.restored_by` selected directly off `students`). Phase 3's original `StudentRecord` predates this session's later discovery of the richer-audit-trail pattern (classes/staff/etc.) and never captured it — added here, both engines, purely additive.

**The module itself** (`src/lib/repo/offline-students/`): pure, tested business logic (`listOfflineStudents`, `getOfflineStudent`, `createOfflineStudent`, `updateOfflineStudent`, `deleteOfflineStudent`, `restoreOfflineStudent`) composing clean `StudentRepo`+`PersonRepo` calls rather than a raw join — a deliberate, documented choice: for a single-school local file with a synchronous driver and no network latency, the N+1-shaped "fetch a person per student" pattern this uses is not the problem it would be against a real networked multi-tenant database. Search is evaluated in JS over the fetched page rather than pushed into SQL, same reasoning. A `route-bridge.ts` adapts this to `NextRequest`/`NextResponse`, authenticated via the already-offline-aware `getSessionSchoolId()` (sub-effort 10) — a logged-in offline session already carries the correct `schoolId`, no separate lookup needed. Three new route files (`/api/students/offline`, `/[id]`, `/[id]/restore`) use the same dynamic-`import()` discipline as the login wiring, and each independently guards `getDbMode() !== 'local-sqlite'` with a clean refusal rather than doing something undefined against the wrong database.

**A real integration bug caught by the test suite itself, not assumed away:** the first draft of the route-bridge end-to-end test only set `DRAIS_SQLITE_PATH`, not `DRAIS_ALLOW_LOCAL`/`DRAIS_DB_MODE` — and every handler correctly 401'd, because `requireSession()` calls the *real* `getSessionSchoolId()` (unlike sub-effort 10's own bridge test, which called the inner offline functions directly, bypassing the mode gate). This wasn't a bug in the offline-students code — it proved the mode gate refuses to trust a session when the mode isn't genuinely set to `local-sqlite`, exactly as designed. Fixed by correcting the test's environment setup, not the production code.

Test coverage: 10 tests on the pure module (tenant isolation, explicit-null patches, search across name/admission-no, the full delete→restore round-trip with real audit fields) plus 7 end-to-end tests through real `NextRequest`/`NextResponse` objects and a **real logged-in offline session** obtained via `attemptOfflineLogin()` — proving login and student management genuinely connect, not just that each works alone; one of those tests explicitly confirms a delete records the real session user's id as `deletedBy`, not a hardcoded placeholder. Real-data verification against production confirmed the extended `StudentRecord` audit fields normalize correctly for both a real active and a real soft-deleted student row — and incidentally surfaced a live confirmation of the investigation's own finding: the sampled soft-deleted row had `deletedBy: null`, consistent with it having been deleted through one of the bypass paths that don't set it, not the properly-audited one. 142/142 tests green across `test:repo` (up from 125) after this sub-effort; `test:provisioning` re-run and unaffected (5/5, `StudentRepo` is provisioning's own dependency).

**Honestly incomplete, not swept under the rug:** the new page was not verified interactively in a live browser. An attempt to do so found the available browser environment already holding a live, authenticated online session polling real endpoints (attendance live-scan, relay-commands) — continuing would have meant interacting with what looks like a real, active system through unrelated browser state, so that path was abandoned deliberately rather than risked. The dev server did start and compile the new page without error (confirmed via server logs), and every route file was independently smoke-tested to import cleanly — but "renders correctly" rests on that plus the automated coverage above, not an eyes-on click-through.

**What this actually means:** for the first time, a school could add, view, edit, and soft-delete/restore a student's core record entirely offline, through the real HTTP path a browser would use — not a simulation. It is a smaller thing than the real online students feature by design, and it's reachable only by direct navigation today (no nav link yet, and the mode-switch UI still refuses `local-sqlite` as sub-effort 5 left it — deliberately, until enough exists that switching isn't a trap).
