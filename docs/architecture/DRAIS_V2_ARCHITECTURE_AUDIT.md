...
**Remaining for Phase 7, not started:** `enrollments`, academic/results tables, `report_snapshots` + DRCE's render-path reads, staff/HR tables, fee tables (last, per the stated priority order), and the `db-mode.ts` three-way-mode wiring itself.
