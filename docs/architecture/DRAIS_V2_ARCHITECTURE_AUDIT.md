...
**Remaining for Phase 7, not started:** `enrollments`, `subjects`/`terms`/`academic_years`/`staff`/`departments`/`subject_groups`/`class_subjects` (the rest of the academic-structure join `report_snapshots` generation needs), `report_snapshots` + DRCE's render-path reads, staff/HR tables, fee tables (last, per the stated priority order), and the `db-mode.ts` three-way-mode wiring itself.
