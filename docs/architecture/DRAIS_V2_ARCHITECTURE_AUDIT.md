...
**Remaining for Phase 7, not started:** `enrollments`, `departments`/`subject_groups`/`class_subjects` (scoped out of sub-effort 4 above, real reasons recorded there), `report_snapshots` + DRCE's render-path reads, fee tables (last, per the stated priority order), the salary/bank-account fields deliberately deferred from `staff` (sub-effort 3, revisit once repo-sqlite has SQLCipher-backed at-rest encryption), and the `db-mode.ts` three-way-mode wiring itself.
