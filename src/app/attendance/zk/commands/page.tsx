...
export default function ZKCommandsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* New Command Form */}
      {showNewForm && (
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-blue-200 dark:border-blue-800 p-6">
      ...
      <div className="flex gap-3">
        <select value={filterDevice} onChange={(e) => { setFilterDevice(e.target.value); setPage(1); }}
          className="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-slate-800">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {commands.length === 0 ? (
                <tr><td colSpan={8} className="text-center text-gray-500 py-12">No commands found</td></tr>
              ) : (
                commands.map((c: any) => (
                  <tr key={c.id} className="border-t dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30">
                    <td className="py-2.5 px-4 font-mono text-xs">#{c.id}</td>
                    <td className="py-2.5 px-4">
                      <span className="font-medium">{c.device_name || c.device_sn}</span>
                    </td>
                    <td className="py-2.5 px-4 font-mono text-xs max-w-[300px] truncate" title={c.command}>
                      {c.command}
                    </td>
                    <td className="py-2.5 px-4">
