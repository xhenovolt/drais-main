...
import {
  Users, Plus, Edit2, Trash2, Save, X, RefreshCw, Search,
...
export default function ZKStudentMappingPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const [showAddForm, setShowAddForm] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [form, setForm] = useState({ device_user_id: '', student_id: '', device_sn: '', card_number: '' });
  ...
  const resetForm = () => {
    setForm({ device_user_id: '', student_id: '', device_sn: '', card_number: '' });
    setShowAddForm(false);
    setEditingId(null);
  };

  const handleSave = async () => {
    if (!form.device_user_id || !form.student_id) {
      toast.error('Device User ID and Student ID are required');
      return;
    }
    setSaving(true);
    try {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Link ZKTeco device user IDs to DRAIS students
          </p>
        </div>
      ...
      {showAddForm && (
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-purple-200 dark:border-purple-800 p-6">
          <h3 className="text-sm font-semibold mb-4">
            {editingId ? 'Edit Mapping' : 'New Student Mapping'}
          </h3>
              ...
              {mappings.length === 0 ? (
                ...
                mappings.map((m: any) => (
                  <tr key={m.id} className="border-t dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30">
                    ...
                    <td className="py-2.5 px-4">Student #{m.student_id}</td>
