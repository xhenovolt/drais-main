 ...
 *
 * Reuses recentModuleStats() (observe.ts) — the read side of the lightweight
 * per-request tap already wired into /api/attendance/history. Only modules
...
export async function observeApiHealth(): Promise<Observation[]> {
  const stats = await recentModuleStats(60);
  ...
  for (const s of stats) {
    ...
    if (s.errorRate >= ERROR_RATE_MEDIUM) {
      observations.push({
        ...
        observer: 'route',
        schoolId: null, // recentModuleStats aggregates across schools; per-school breakdown is a future refinement
        module: s.module,
        severity: s.errorRate >= ERROR_RATE_HIGH ? 'high' : 'medium',
        confidence: Math.min(95, 60 + s.count),
        probableCause: `${Math.round(s.errorRate * 100)}% of the last ${s.count} observed request(s) to ${s.module} returned a server error (5xx).`,
        ...
        dedupKey: `api_error_rate_anomaly::global::${s.module}`,
      });
    }
    ...
    if (s.p95DurationMs >= P95_LATENCY_HIGH_MS) {
      observations.push({
        ...
        probableCause: `p95 response time for ${s.module} is ${s.p95DurationMs}ms over the last ${s.count} observed request(s).`,
        ...
        dedupKey: `api_latency_anomaly::global::${s.module}`,
      });
    }
  }
