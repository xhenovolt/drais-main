  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="font-semibold text-gray-900 dark:text-white">{isAr ? 'حضور اليوم' : 'Attendance Today'}</h3>
      ...
      {isLoading && (
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">{isAr ? 'جارٍ تحميل الحضور…' : 'Loading attendance…'}</p>
        </div>
      )}
      ...
      {error && !isLoading && (
        <div className="flex items-center gap-2 text-red-600 dark:text-red-400 py-4">
          ...
          <span className="text-sm">{isAr ? 'تعذّر تحميل بيانات الحضور' : 'Failed to load attendance data'}</span>
        </div>
      )}
      ...
      {!isLoading && !error && !stats && (
        <div className="text-center py-8 text-gray-400 dark:text-gray-500">
          ...
          <p className="text-sm">{isAr ? 'لا توجد سجلات حضور لليوم' : 'No attendance records for today'}</p>
          <Link href="/attendance" className="mt-2 inline-block text-xs text-teal-600 hover:underline">
            {isAr ? 'تسجيل الحضور ←' : 'Mark attendance →'}
          </Link>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
              ...
              <p className="text-xs text-green-600 dark:text-green-400">{isAr ? 'حاضر' : 'Present'}</p>
            </div>
            <div className="text-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
              <p className="text-lg font-bold text-red-700 dark:text-red-300">{stats.absent ?? 0}</p>
              <p className="text-xs text-red-600 dark:text-red-400">{isAr ? 'غائب' : 'Absent'}</p>
            </div>
            <div className="text-center p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
              <p className="text-lg font-bold text-yellow-700 dark:text-yellow-300">{stats.late ?? 0}</p>
              <p className="text-xs text-yellow-600 dark:text-yellow-400">{isAr ? 'متأخر' : 'Late'}</p>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-gray-100 dark:border-slate-700">
            <Link href="/attendance" className="text-sm text-teal-600 hover:text-teal-700 dark:hover:text-teal-400 transition-colors">
              {isAr ? 'عرض الحضور الكامل ←' : 'View full attendance →'}
            </Link>
          </div>
        </div>
      )}
